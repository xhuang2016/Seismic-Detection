...
"""
====================================
k-fold Cross Validation for CRNN
====================================
"""
import pandas as pd
import numpy as np
...
for (train_index, test_index), (train_index2, test_index2) in zip(kf.split(X_EQ), kf.split(X_HA)):
    ...
    ## save ouput prediction probability
    result = np.concatenate((y_test, y_prob.reshape(-1, 1)),axis=1)
    df_result = pd.DataFrame(result,columns=['labels','prob_1'])
    df_result.to_csv(write_path + 'CRNN_%s'%SamplingRate + 'Hz_%s'%Duration +'s_fold_%s.csv'% i,index=False)
    i += 1
